...
def get_realtime_alarm(username, auth, url):
    ...
    try:
        ...
        if (realtime_alarm_list['count'] == "1"):
...
def get_alarm_byId (alarm_id, auth, url):
    get_alarm_byId_url = "/imcrs/fault/alarm/" + alarm_id
    f_url = url + get_alarm_byId_url
    r = requests.get(f_url, auth=auth, headers=headers)
    try:
        if r.status_code == 200:
            alarm_info = (json.loads(r.text))
            return alarm_info
    except requests.exceptions.RequestException as e:
            return "Error:\n" + str(e) + ' get_alarm_byId: An Error has occured'